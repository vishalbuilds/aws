...
from src import Auto_disconnect_longer_contact as handler

class TestLambdaFunction(unittest.TestCase):

    @patch('src.Auto_disconnect_longer_contact.connect_client')
    ...
    def test_disconnect_contact(self, mock_connect_client):
        ...
        with patch('src.Auto_disconnect_longer_contact.describe_contact', return_value={'data': {'status': 'Complete'}}):
            result = handler.disconnect_contact('contact1')
            self.assertEqual(result['data']['contact_status'], 'Successful')

    @patch('src.Auto_disconnect_longer_contact.fetch_routing_profiles_arn', return_value=['arn1'])
    @patch('src.Auto_disconnect_longer_contact.fetch_contacts', return_value=['contact1'])
    @patch('src.Auto_disconnect_longer_contact.describe_contact', return_value={'data': {'LastUpdateTimestamp': datetime.now(timezone.utc).isoformat()}})
    @patch('src.Auto_disconnect_longer_contact.should_disconnect', return_value=True)
    @patch('src.Auto_disconnect_longer_contact.disconnect_contact', return_value={'data': {'contact_status': 'Successful'}})
